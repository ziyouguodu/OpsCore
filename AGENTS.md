...
- AI Copilot 配置：一期前端提供模型厂商、端点、模型、上下文授权和审计边界配置界面，后续再接入后端密钥托管与调用代理。
...
- AI Copilot 配置页属于“系统配置”，支持本地模型、OpenAI GPT、Anthropic Claude、Google Gemini 和 OpenAI 兼容接口；生产密钥不得落前端。
